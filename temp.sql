...
SELECT temp_data.plate,  temp_data.well,  temp_data.response ,plate_plate_set.plate_order , plate_set.plate_set_sys_name , plate.plate_sys_name, well.id, well.well_name, sample.sample_sys_name FROM temp_data, plate_plate_set, plate_set, plate, well,sample, well_sample, well_numbers WHERE temp_data.plate = plate_plate_set.plate_order AND plate_plate_set.plate_id = plate.id AND well.plate_id = plate.id AND well_sample.well_id = well.id AND well_sample.sample_id = sample.id AND plate_plate_set.plate_set_id = plate_set.id AND plate_plate_set.plate_set_id = 10 AND temp_data.well = well_numbers.by_col AND well_numbers.well_name = well.well_name AND well_numbers.plate_format = 1536 ORDER BY plate_plate_set.plate_order, well_numbers.by_col;
